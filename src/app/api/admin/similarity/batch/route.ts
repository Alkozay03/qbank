...
export async function POST(request: Request) {
  ...
  console.error(`🚀🚀🚀 [BATCH-V2] FUNCTION ENTRY POINT - ${new Date().toISOString()}`);
  
  try {
    // Check authorization
    console.error(`🚀 [BATCH-V2] Checking authorization...`);
    await requireRole(["ADMIN", "MASTER_ADMIN", "WEBSITE_CREATOR"]);
    console.error(`🚀 [BATCH-V2] Authorization passed!`);
