...
export async function GET() {
  ...
  try {
    const [totalQuestions, correctResponses, totalResponses, testsCompleted, uniqueQuestionsSolved] = await Promise.all([
      // Only count PreClerkship questions for Year 2
      prisma.preClerkshipQuestion.count({
        where: {
          yearLevel: 2,
        },
      }),
      prisma.preClerkshipResponse.count({
        where: {
          PreClerkshipQuizItem: { 
            PreClerkshipQuiz: { userId, yearLevel: 2, status: "Ended" } 
          },
          isCorrect: true,
        },
      }),
      ...
      prisma.preClerkshipQuiz.count({ where: { userId, yearLevel: 2, status: "Ended" } }),
      // Count unique QUESTIONS the user has answered for Year 2
      prisma.preClerkshipQuizItem.findMany({
        where: { 
          PreClerkshipQuiz: { userId, yearLevel: 2, status: "Ended" },
          PreClerkshipResponse: { some: {} }
        },
        select: {
          questionId: true
        },
    ...
    console.error("[Year2 Dashboard Stats] Error:", error);
