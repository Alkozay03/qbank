...
export async function GET() {
  ...
  try {
    const [totalQuestions, correctResponses, totalResponses, testsCompleted, uniqueQuestionsSolved] = await Promise.all([
      // Only count PreClerkship questions for Year 1
      ...
      prisma.preClerkshipResponse.count({
        where: {
          PreClerkshipQuizItem: { 
            PreClerkshipQuiz: { userId, yearLevel: 1, status: "Ended" } 
          },
          isCorrect: true,
        },
      }),
      ...
      prisma.preClerkshipQuiz.count({ where: { userId, yearLevel: 1, status: "Ended" } }),
      // Count unique QUESTIONS the user has answered for Year 1
    ...
    console.error("[Year1 Dashboard Stats] Error:", error);
