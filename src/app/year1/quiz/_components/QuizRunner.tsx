...
type TagType = "SUBJECT" | "SYSTEM" | "TOPIC" | "ROTATION" | "RESOURCE" | "MODE" | "WEEK" | "LECTURE";
...
export default function QuizRunner({ initialQuiz }: { initialQuiz: InitialQuiz }) {
  ...
  async function toggleFlag(marked: boolean) {
    if (!currentItem) return;
    try {
      await fetch(`/api/preclerkship/quiz/${id}/flag`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  async function submitAnswer() {
    ...
    try {
      const res = await fetch(`/api/preclerkship/quiz/${id}/submit`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  async function endQuiz() {
    try {
      await fetch(`/api/preclerkship/quiz/${id}/end`, { method: "POST" });
      setStatus("Ended");
    } finally {
      setConfirmEnd(false);
  ...
  async function suspendQuiz() {
    try {
      await fetch(`/api/preclerkship/quiz/${id}/suspend`, { method: "POST" });
      setStatus("Suspended");
    } finally {
