  ...
  const handleAddManualQuestion = useCallback(async () => {
    try {
      ...
      const response = await fetch('/api/preclerkship/admin/questions/draft', {
        ...
        body: JSON.stringify({ yearLevel: 3 }),
      });
...
${formattedList}`);
      ...
      console.warn('🔷 [SAVE ALL] API payload:', {
        url: '/api/preclerkship/admin/questions/bulk',
        method: 'POST',
        questionCount: apiPayload.questions.length,
        payloadSize: JSON.stringify(apiPayload).length,
      });

      const response = await fetch('/api/preclerkship/admin/questions/bulk', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleGoBack = () => {
    ...
    console.warn('🔍 [BULK Q MANAGER] Will navigate to:', userRole === "MASTER_ADMIN" ? "/year3/master-admin" : "/year3/admin");
    if (userRole === "MASTER_ADMIN") {
      router.push("/year3/master-admin");
    } else {
      router.push("/year3/admin");
    }
  };
