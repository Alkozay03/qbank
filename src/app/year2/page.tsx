...
import DashboardStatsClient from "@/components/year2/DashboardStatsClient";
  ...
  if (session?.user?.email) {
    try {
      ...
      if (typeof process !== 'undefined' && process.stderr) {
        process.stderr.write(`❌ [YEAR2] Database query failed: ${error}\n`);
        if (error instanceof Error) {
          process.stderr.write(`❌ [YEAR2] Error message: ${error.message}\n`);
          process.stderr.write(`❌ [YEAR2] Error stack: ${error.stack}\n`);
        }
      }
