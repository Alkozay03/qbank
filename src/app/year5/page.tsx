...
import DashboardStatsClient from "@/components/year5/DashboardStatsClient";
  ...
  if (session?.user?.email) {
    try {
      ...
      if (typeof process !== 'undefined' && process.stderr) {
        process.stderr.write(`❌ [YEAR5] Database query failed: ${error}\n`);
        if (error instanceof Error) {
          process.stderr.write(`❌ [YEAR5] Error message: ${error.message}\n`);
          process.stderr.write(`❌ [YEAR5] Error stack: ${error.stack}\n`);
        }
      }
