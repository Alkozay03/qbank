...
export default function ViewQuestionsPage() {
  ...
  // keep topic selections aligned with chosen rotations
  useEffect(() => {
    setSelTopics((prev) => prev.filter((key) => allowedTopicKeys.has(key)));
  }, [allowedTopicKeys]);

  // auto-include "Not Selected" topic when applicable
  useEffect(() => {
    ...
    try {
      ...
      const normalized = Array.isArray(payload?.questions)
        ? payload.questions.map((q: QuestionRow) => {
            ...
            const created = (q as any).createdAt ?? q.updatedAt ?? fallback;
            const updated = q.updatedAt ?? (q as any).createdAt ?? fallback;
  ...
  const handleEdit = (id: string, customId: number | null) => {
    ...
    router.push(`/year5/admin/bulk-question-manager?questionId=${targetId}`);
            ...
            <div className="flex flex-wrap gap-3">
              <button
                onClick={() => {
                  ...
                  router.push(`/year5/admin/bulk-question-manager?questionIds=${encodeURIComponent(ids.join(","))}`);
                }}
